...
const getAllRoles = async (req, res) => {
    ...
    try {
        ...
        for (const role of rolesResult.rows) {
            const permsResult = await client.query(`SELECT p.name FROM permissions p
...
const createRole = async (req, res) => {
    ...
    try {
        ...
        const roleResult = await client.query(`INSERT INTO roles (name, display_name, description, is_system, is_active, created_by)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`, [name, displayName, description, false, isActive ?? true, userId]);
        const role = roleResult.rows[0];
        if (permissions && permissions.length > 0) {
            for (const permName of permissions) {
                const permResult = await client.query('SELECT id FROM permissions WHERE name = $1', [permName]);
                if (permResult.rows.length > 0) {
                    await client.query('INSERT INTO role_permissions (role_id, permission_id, granted_by) VALUES ($1, $2, $3)', [role.id, permResult.rows[0].id, userId]);
                }
            }
        }
...
const updateRole = async (req, res) => {
    ...
    try {
        ...
        if (updates.length > 0) {
            ...
            await client.query(`UPDATE roles SET ${updates.join(', ')} WHERE id = $${paramCount}`, values);
        }
        if (permissions) {
            ...
            for (const permName of permissions) {
                const permResult = await client.query('SELECT id FROM permissions WHERE name = $1', [permName]);
                if (permResult.rows.length > 0) {
                    await client.query('INSERT INTO role_permissions (role_id, permission_id, granted_by) VALUES ($1, $2, $3)', [req.params.id, permResult.rows[0].id, userId]);
                }
            }
        }
        await client.query('COMMIT');
