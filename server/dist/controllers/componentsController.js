...
const getMyComponents = async (req, res) => {
    ...
    try {
        ...
        const userResult = await client.query(`SELECT role, role_id FROM users WHERE id = $1 LIMIT 1`, [req.user.userId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const roleName = userResult.rows[0]?.role ?? null;
        let roleId = userResult.rows[0]?.role_id ?? null;
        if (!roleId && roleName) {
            try {
                const fallbackRole = await client.query(`SELECT id FROM roles WHERE name = $1 LIMIT 1`, [roleName]);
