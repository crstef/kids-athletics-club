    ...
    try {
      ...
      WHERE ar.id = $1`;
        ...
        if (status === 'approved') {
            ...
            const userUpdateValues = [approverId, requestRow.parent_id];
            let valueIndex = 3;
            let userUpdateSet = `
        is_active = true,
        ...
        role = COALESCE(role, 'parent')`;
            ...
            await client.query(`UPDATE users SET ${userUpdateSet} WHERE id = $2`, userUpdateValues);
            if (requestRow.athlete_id) {
                await client.query(`UPDATE athletes SET parent_id = $1 WHERE id = $2`, [requestRow.parent_id, requestRow.athlete_id]);
            }
         ...
         WHERE user_id = $2
           ...
           AND status = 'pending'
           AND (coach_id = $3 OR coach_id IS NULL)`, [approverId, requestRow.parent_id, requestRow.coach_id]);
        }
