...
export const createAdminUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const superadminRoleId = superadminRole.rows[0].id as string;
 ...
 */
export const fixUserRoles = async (req: Request, res: Response) => {
  const client = await pool.connect();
 ...
 */
export const addModernDashboards = async (_req: Request, res: Response) => {
  const client = await pool.connect();
 ...
 */
export const populateRoleDashboards = async (req: Request, res: Response) => {
  const client = await pool.connect();
  ...
  try {
    ...
    for (const role of roles) {
      ...
      const dashboardResult = await client.query(
        'SELECT id FROM dashboards WHERE name = $1 AND is_active = true',
        [dashboardName]
      );
      ...
      const existingResult = await client.query(
        'SELECT id FROM role_dashboards WHERE role_id = $1 AND dashboard_id = $2',
        [role.id, dashboardId]
      );
...
export const resetDatabase = async (req: Request, res: Response) => {
  ...
  const _athlete2 = await client.query('SELECT id FROM users WHERE email = $1', ['athlete2@kidsathletics.ro']);
    ...
    if (coach1.rows.length > 0 && parent1.rows.length > 0 && athlete1.rows.length > 0) {
      ...
      await client.query(
        `INSERT INTO athletes (first_name, last_name, age, category, gender, date_of_birth, date_joined, coach_id, parent_id)
         VALUES 
      ...
      if (athleteRecords.rows.length >= 2 && athlete1.rows.length > 0) {
        await client.query(
          ...
          [athleteRecords.rows[0].id, 'athlete1@kidsathletics.ro']
        );
        await client.query(
          'UPDATE users SET athlete_id = $1 WHERE email = $2',
 ...
 */
export const fixRoleDashboardsSchema = async (req: Request, res: Response) => {
  const client = await pool.connect();
...
export const createUserWidgetsTable = async (req: Request, res: Response) => {
  const client = await pool.connect();
